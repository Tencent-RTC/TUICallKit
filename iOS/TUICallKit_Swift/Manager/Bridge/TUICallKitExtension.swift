...
class TUICallKitExtension: NSObject, TUIExtensionProtocol {
    ...
    func onRaiseExtension(_ extensionID: String, parentView: UIView, param: [AnyHashable : Any]?) -> Bool {
        ...
        if extensionID == TUICore_TUIChatExtension_ChatViewTopArea_ClassicExtensionID ||
            ...
            let joinCallView = JoinCallView()
            JoinCallViewManager.shared.setJoinGroupCallView(joinCallView)
            JoinCallViewManager.shared.getGroupAttributes(groupId)
            parentView.subviews.forEach {
                if $0 is JoinCallView {
                    $0.removeFromSuperview()
                }
            }
            parentView.addSubview(joinCallView)
            return true
        }
    ...
    func getNavigationMoreItemExtensionForMinimalistChat(param: [AnyHashable : Any]?) -> [TUIExtensionInfo]? {
        ...
        if !filterVideoCall {
            ...
            videoInfo.icon = CallKitBundle.getBundleImage(name: "video_call") ?? UIImage()
        ...
        if !filterAudioCall {
            ...
            audioInfo.icon = CallKitBundle.getBundleImage(name: "audio_call") ?? UIImage()
